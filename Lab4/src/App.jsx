...
function App() {
  ...
  // Передаємо параметри з нашого комплексного стану у кастомний хук
  const { data: sensorData, resetScanner } = useSensorData(appState.isRunning, appState.intervalMs);
  ...
  // Ефект ТІЛЬКИ для малювання графіка
  useEffect(() => {
  ...
  // Функція запуску/зупинки системи (оновлює одразу кілька полів у стані)
  ...
  const handleSave = (e) => {
    ...
    setAppState(prev => ({
      ...
      intervalMs: i !== "" ? Number(i) * 1000 : prev.intervalMs,
      logs: [`[${new Date().toLocaleTimeString()}] — ${logMsg}`, ...prev.logs],
      formValues: { deviceName: '', interval: '' } // очищаємо форму після збереження
    }));
  };
                ...
                <div className={cardClasses}>
                  ...
                  <form onSubmit={handleSave} className="flex flex-col gap-3">
                    ...
                    <input 
                      ...
                      placeholder="1000" 
                    />
                    <button type="submit" className={`font-bold py-2 mt-2 rounded-lg hover:opacity-90 uppercase tracking-wider transition-colors duration-300 ${appState.isDark ? 'bg-dt-violet text-dt-zinc' : 'bg-lt-violet text-white'}`}>
                      Save
                    </button>
                  </form>
                </div>
